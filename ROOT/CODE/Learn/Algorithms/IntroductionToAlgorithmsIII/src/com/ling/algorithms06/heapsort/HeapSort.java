...
public class HeapSort {
	public static void main(String[] args) {
		...
		HeapUtil.heapify(new HeapObject(data, data.length, true), 1);
		System.out.println(Arrays.toString(data));
		...
		// 大顶堆
		HeapUtil.buildHeap(data2, true);
		System.out.println(Arrays.toString(data2));
		// 小顶堆
		HeapUtil.buildHeap(data2, false);
		System.out.println(Arrays.toString(data2));
	 ...
	 * @param ascFlag-正序标识
	 */
	public static void heapSort(Double[] data, boolean ascFlag) {
		HeapUtil.buildHeap(data, ascFlag);// 首先用待排序数组建堆
		...
		while (heapSize > 1) {
			...
			--heapSize;
			// 对新的根节点维护堆性质
			HeapUtil.heapify(new HeapObject(data, heapSize, ascFlag), 0);
		}
