...
public class Chapter02 {
	...
	public void testLoginCookies(Jedis conn) throws InterruptedException {
		...
		String r = checkToken(conn, token);//通过token获取对应的用户名
	...
	public void testShopppingCartCookies(Jedis conn) throws InterruptedException {
		...
		addToCart(conn, token, "itemY", 3);//加入购物车
	...
	public void updateToken(Jedis conn, String token, String user, String item) {
		...
		if (item != null) {
			...
			/*
			 * Redis Zincrby 命令对有序集合中指定成员的分数加上增量 increment 可以通过传递一个负数值 increment
