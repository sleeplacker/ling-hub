...
> `openssl req -new -key private/ca.key -out private/ca.csr -subj "/C=MO/ST=MO/L=MO/O=MO/OU=MO/CN=51666"`
...
> `openssl req -new -key private/server.key -out private/server.csr -subj "/C=MO/ST=MO/L=MO/O=MO/OU=MO/CN=51888"`



3. 签发服务器证书(使用上面生成的CA根证书)

> `openssl x509 -req -days 3650 -sha256 -extensions v3_req -CA certs/ca.cer -CAkey private/ca.key -CAserial ca.srl -CAcreateserial -in private/server.csr -out certs/server.cer`
>
> 这里的ca.srl是CA证书序列化文件
...
> `openssl req -new -key private/client.key -out private/client.csr -subj "/C=MO/ST=MO/L=MO/O=MO/OU=MO/CN=52001"`
...
>
> 注意这里需要在工作目录下面创建index.txt和serial两个文件，serial文件中是一个数字，证书签发成功后，改数字会+1
